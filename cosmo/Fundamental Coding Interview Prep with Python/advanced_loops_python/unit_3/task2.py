...
2. If the value in the current position is zero, then the game ends. If the player's current position leads them outside of the array's boundaries,
   then their ability to move in the current direction ceases.
3. If the latter happens, then the player reverses their direction and continues to move according to similar rules, but now the directions are inverted:
...
You are to implement a function titled evaluatePath(numbers).
This function should take an array of integers as input, representing the path and its rules, and return a tuple (position, moves), where:
    ...
    - It's guaranteed that the game will not lead to an infinite loop,
      (i.e. the path to the next blockade or the array boundaries would not require the player to visit the same position more than once.)

For instance, given an array [3, 4, 1, 1, -3, 1]. The output would be (4, 5). Here's how it works:

1. The player starts at position 0, where the value is 3. They move 3 steps to the right and land on the 3rd position. Total moves till now: 1.
