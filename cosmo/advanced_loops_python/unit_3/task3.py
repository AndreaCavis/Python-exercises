...
The game is played on a linear board represented by an array of positive integers. The length of the board can range from 1 to 500 inclusive. 
Each position on the board represents the number of steps a player can make from that position. 
The player can only move towards the end of the board. The board can contain obstacles on which the player cannot land, defined by a specific integer value. 
The game ends when the player exits the board or lands on an obstacle.

Your task is to implement the solution(board, obstacle) function, which returns an array moves. 
For every i in moves, the algorithm should calculate the number of moves required for a player to exit the board,
while starting from the i-th position, without landing on an obstacle. 
